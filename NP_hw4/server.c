...
//	struct Data database[20];
	...
	for(j = 0; j < 100; j++){
		...
		database[j].sub_boards = 0;
		database[j].sub_authors = 0;
		for(int k = 0; k < 101; k ++){
...
void* conn(void *arg){
	...
	if(tmpp > 0){
		...
		else if(!strncmp(recv_msg, "subscribe --", 12)){
			if(login_yn == 0){	//login first
				send(fd, ERR6, strlen(ERR6), 0);
			}
			else if(!strncmp(recv_msg, "subscribe --board", 17)){
				if(strstr(recv_msg, "--keyword") != NULL){ //correct format
					char *board = strdup(recv_msg + 18);
					char *keyword = strstr(recv_msg, "--keyword");
					int len = strlen(board) - strlen(keyword);
					char board_name[100];	//real board name
